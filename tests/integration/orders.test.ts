...
import { expectResponseTime, expectSuccessResponse, expectValidOrder } from "../helpers/assertions";
...
describe("Order Endpoints", () => {
  ...
  describe("POST /order (Create Order)", () => {
    ...
    it("should generate unique order IDs", async () => {
      const requests = Array(5)
        .fill(null)
        .map(() => client.createOrder(createValidOrderRequest()));
    ...
    it("should respond within 1000ms", async () => {
      const orderRequest = createValidOrderRequest();
      const { responseTime } = await measureResponseTime(() => client.createOrder(orderRequest));
  ...
  describe("GET /order/:id (Get Order)", () => {
    ...
    it("should respond within 500ms", async () => {
      const { responseTime } = await measureResponseTime(() => client.getOrder(testOrderId));
    ...
    it("should handle multiple requests for same order", async () => {
      const requests = Array(5)
        .fill(null)
        .map(() => client.getOrder(testOrderId));
      const responses = await Promise.all(requests);
